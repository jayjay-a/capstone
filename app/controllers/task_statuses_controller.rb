class TaskStatusesController < ApplicationController
  ...
  def index
    if user_signed_in?
      ...
      @task_statuses = @search.result.order("created_at DESC").page(params[:page]).per(50)
    else
  ...
  def destroy
    @task_status.destroy
    respond_to do |format|
      format.html { redirect_to task_statuses_url, notice: 'Task status was successfully deleted.' }
  ...
  private
    ...
    def task_status_params
      params.require(:task_status).permit(:task_status_description)
    end
end
