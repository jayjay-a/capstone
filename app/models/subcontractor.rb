class Subcontractor < ApplicationRecord
  ...
  validates :subcontractor_email, allow_blank: true, length: { maximum: 70 }, format: { with: /\A([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})\z/i, on: :create, on: :update, message: 'is not a valid email address' }
