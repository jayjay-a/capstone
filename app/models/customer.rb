class Customer < ApplicationRecord
  ...
  validates :customer_rep_name_2, allow_nil: true, length: { maximum: 70 }, format: { with: /\A(?=.* )[^0-9`!@#\$%\^&*+_=(){}]+\z/ }
  validates :customer_rep_phone_1, allow_nil: true, length: { maximum: 15 }, format: { with: /\A\(?\d{3}\)?[- ]?\d{3}[- ]?\d{4}\z/, message: 'needs to only be numbers and -\'s or ()\'s' }
  validates :customer_rep_phone_2, allow_nil: true, length: { maximum: 26 }, format: { with: /\A\(?\d{3}\)?[- ]?\d{3}[- ]?\d{4}\z/, message: 'needs to only be numbers and -\'s or ()\'s' }
  ...
  validates :customer_city, allow_nil: true, length: { maximum: 64 }, format: { with: /\A[^0-9`!@#\$%\^&*+_=(){}]+\z/ }
