class Customer < ApplicationRecord
  ...
  validates :customer_rep_name_1, length: { maximum: 70 }, format: { with: /\A[a-zA-Z]+(?: [a-zA-Z]+)?\z/, message: 'can only be letters' }
  validates :customer_rep_name_2, allow_blank: true, length: { maximum: 70 }, format: { with: /\A[a-zA-Z]+(?: [a-zA-Z]+)?\z/, message: 'can only be letters' }
  ...
  validates :customer_city, allow_blank: true, length: { maximum: 64 }, format: { with: /\A[a-zA-Z]+(?: [a-zA-Z]+)?\z/, message: 'can only be letters' }
